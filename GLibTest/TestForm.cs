...
namespace GLibTest
{
	public partial class TestForm : Form
	{
		...
		private void d2dControl1_OnRendering(object sender, ID2D1DeviceContext g)
		{
			...
			_ = Enum.TryParse(cStrokeStyle.SelectedItem?.ToString(), out DashStyle dashStyle);
			...
			{
				strokeStyle2 = bitmapRenderTarget.Factory.CreateStrokeStyle(new StrokeStyleProperties
				{
		...
		{
			// iterate through enum name StrokeStyle and add all values in combobox cstrokestyle
			foreach (var item in Enum.GetNames(typeof(DashStyle)))
			{
				cStrokeStyle.Items.Add(item);
			}
		}

		private void cStrokeStyle_SelectedIndexChanged(object sender, EventArgs e)
		{
